...
class UserTest < ActiveSupport::TestCase
  ...
  test '1.1.4 email validation should accept valid addresses' do
    valid_addresses =
      %w(user@exempel.se USER@example.com A_US-ER@e.mail.org first.last@foo.jp kalle+lisa@baz.cn)
  ...
  test '1.1.5 email validation should reject invalid addresses' do
    invalid_addresses =
      %w(user@example,com user_at_mail.org user.name@example. user@e_mail.com user@e+post.com user@mail..com)
  ...
  # Test uses reload method to see if value from database is
  # equal to lower-case value.
